...
INTERVIEW_QUESTIONS = {
    "ndcg": "Walk me through how you measured ranking quality in production.",
    "information retrieval": "Describe a retrieval system you shipped. What was the hardest scaling challenge?",
    ...
    "faiss": "When would you choose FAISS over a managed vector DB like Pinecone?",
    "pinecone": "Describe managing a vector database at scale. How did you handle index updates?",
    "weaviate": "What factors did you consider when selecting a vector database?",
    "learning to rank": "Walk me through a learning-to-rank model you trained end to end.",
    "hybrid search": "How do you combine BM25 and dense scores? What fusion strategy?",
    "a/b test": "Describe an A/B test on a search/ranking system. What was your primary metric?",
    "evaluation framework": "How do you build offline evaluation for retrieval? How do you get relevance labels?",
    "lambdamart": "Why LambdaMART over neural ranking? What were the latency/quality tradeoffs?",
    "lora": "How have you used LoRA/QLoRA for fine-tuning? What did you measure?",
    "rag": "Describe the retrieval component of a RAG system you built.",
    "recommendation": "How did you evaluate your recommendation system?",
    "python": "Show me a piece of Python code from a production ML system you are proud of.",
}
BIAS_FLAGS = {
    ...
    "young":          "Age-coded language — potentially discriminatory",
    "digital native": "Age-coded language — implies preference for younger candidates",
    "energetic":      "Can be used as proxy for age — older candidates self-select out",
...
def score_one(c):
    """Score a single candidate. Returns (composite, skills, career, beh) or None if filtered."""
    ...
    if all_cos and all(any(cc in co for cc in CONSULTING) for co in all_cos):
        career_score *= 0.4
    ...
    return composite, skills_score, career_score, beh

def build_gap_analysis(candidate):
    ...
    present, missing = [], []
    for token, weight in sorted(JD_REQUIRED_SKILLS.items(), key=lambda x: -x[1])[:12]:
...
def rank_streaming(uploaded_file, top_n=20):
    """
    KEY INSIGHT: Score each candidate immediately as we read it.
    Only keep top_n candidates in memory at any time using a heap.
    Memory usage = top_n candidates, not 100K.
    """
    import heapq

    # Write to temp file in chunks to avoid RAM spike
    progress = st.progress(0, text="Saving file to disk...")
    with tempfile.NamedTemporaryFile(
        ...
        while True:
            ...
            if file_size:
                pct = min(int(bytes_written / file_size * 40), 39)
                progress.progress(
                    pct,
                    text=f"Uploading... {bytes_written/1024/1024:.0f} / {file_size/1024/1024:.0f} MB"
                )

    progress.progress(40, text="Processing candidates...")

    # Stream from disk, score immediately, keep only top_n
    # heap stores (-score, index, snapshot) — negative because heapq is min-heap
    heap = []
    total_read = 0
    total_scored = 0
    ...
    if is_json:
        ...
        for c in all_candidates:
            scored = score_one(c)
            if scored:
                composite, skills_score, career_score, beh = scored
                results.append({
                    "candidate": c,
                    "composite": composite,
                    "skills": skills_score,
        ...
        progress.progress(100, text=f"Done — {len(all_candidates):,} candidates ranked")
        return results[:top_n], len(all_candidates)

    # JSONL streaming — score + heap, never hold all candidates in RAM
    with open(tmp_path, 'r', encoding='utf-8', errors='ignore') as f:
        for i, line in enumerate(f):
            ...
            # Keep a heap of top_n candidates
            # Store minimal snapshot to save memory
            snapshot = {
            ...
            if len(heap) < top_n:
                heapq.heappush(heap, (composite, total_read, snapshot,
                                      skills_score, career_score, beh))
            elif composite > heap[0][0]:
                heapq.heapreplace(heap, (composite, total_read, snapshot,
                                         skills_score, career_score, beh))

            if total_read % 5000 == 0:
                pct = 40 + min(int(total_read / 100000 * 58), 58)
                progress.progress(
                    pct,
                    text=f"Scoring... {total_read:,} read, {total_scored:,} scored"
                )

    os.unlink(tmp_path)
    progress.progress(100, text=f"Done — {total_read:,} candidates processed")

    # Sort heap results by score descending
...
st.markdown("""
<style>
.tag-match{background:#E1F5EE;color:#085041;padding:2px 8px;border-radius:10px;
           font-size:12px;margin:2px;display:inline-block}
.tag-gap{background:#FCEBEB;color:#791F1F;padding:2px 8px;border-radius:10px;
         font-size:12px;margin:2px;display:inline-block}
</style>
...
with col_left:
    ...
    jd_text = st.text_area(
        ...
        value="Senior AI/ML Engineer with expertise in retrieval systems, vector "
              "databases (Pinecone/Weaviate/FAISS), semantic search, learning to rank, "
              "and evaluation frameworks (NDCG/MRR). Must have Python and 5+ years in "
              "production ML.",
        height=140,
    ...
    if bias_issues:
        st.warning(f"Bias check: {len(bias_issues)} flag(s) found in JD")
    ...
    st.caption("Supports sample_candidates.json (50 candidates) or candidates.jsonl (100K)")
...
with col_right:
    if run_btn and uploaded:
        t_start = time.time()
        results, total_loaded = rank_streaming(uploaded, top_n=top_n)
        t_elapsed = time.time() - t_start

        st.divider()
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Candidates", f"{total_loaded:,}")
        m2.metric("Shortlisted", len(results))
        m3.metric("Top score", f"{results[0]['composite']:.3f}" if results else "—")
        m4.metric("Time", f"{t_elapsed:.1f}s")
        ...
        for i, item in enumerate(results):
            ...
            with st.expander(
                ...
                with col_info:
                    ...
                    if edu:
                        st.markdown(
                            f"**Education:** {edu[0].get('degree', '')} — "
                            f"{edu[0].get('field_of_study', '')}"
                        )
                with col_signals:
                    st.markdown(f"**Response rate:** {rr:.0%} {'ok' if rr >= 0.5 else 'low'}")
                    st.markdown(f"**GitHub:** {gh if gh >= 0 else 'not linked'}")
                    st.markdown(f"**Notice:** {notice} days")
                    st.markdown(f"**Open to work:** {'Yes' if otw else 'No'} | Last active: {last_active}")

                present, missing, questions = build_gap_analysis(c)
                st.markdown("**JD skill alignment:**")
                tag_html = ""
                for s in present[:6]:
                    tag_html += f'<span class="tag-match">ok {s}</span> '
                for s in missing[:4]:
                    tag_html += f'<span class="tag-gap">missing {s}</span> '
                st.markdown(tag_html, unsafe_allow_html=True)

                if questions:
                    st.markdown("**Interview questions for skill gaps:**")
        ...
        if results:
            ...
            for i, item in enumerate(results):
                ...
                reasoning = (
                    f"{p['years_of_experience']:.0f}-year {p['current_title']} "
...
2. **Semantic career fit** — searches role descriptions for production evidence, not just keywords
3. **Behavioral signals** — 8 engagement signals applied as a composite multiplier
...
Supports both `sample_candidates.json` (50 candidates, instant) and `candidates.jsonl` (100K, ~90s).
        """)
        st.info("The full 100K ranking runs locally: python rank.py --candidates candidates.jsonl --out submission.csv")
